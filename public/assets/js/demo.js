    ...
    options: {
        ...
        scales: {
            yAxes: [{
                ticks: {
                    fontStyle: "500",
                    beginAtZero: true, // Adjust as necessary
                    maxTicksLimit: 5,
                    padding: 10
                },
                gridLines: {
...
async function fetchChartData() {
    try {
        ...
        chartData.sort((a, b) => a.angkatan - b.angkatan);

        const labels = chartData.map(item => item.angkatan);
        const data = chartData.map(item => item.jumlah);
        const backgroundColors = chartData.map((item, index) => {
            const colors = ['#f3545d', '#fdaf4b', '#177dff', '#1d7a46', '#ffcd56', '#36a2eb', '#ff9f40', '#ff6384'];
